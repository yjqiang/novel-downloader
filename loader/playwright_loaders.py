...
class PlaywrightBodyPageLoader(BodyPageLoader):
    @staticmethod
    async def fetch_html(session: playwright_websession.WebSession, website_setting_rule: WebsiteSettingRule, body_page_data: BodyPageData) -> None:
        return await _fetch_html(session, website_setting_rule, body_page_data)
    ...
    @staticmethod
    async def goto_next_page(session: playwright_websession.WebSession, website_setting_rule: WebsiteSettingRule, cur_body_page: BodyPageData) -> BodyPageData:
        next_page_data = BodyPageData(url=cur_body_page.next_page_url, page_id=cur_body_page.page_id + 1)
        await _goto_next_page(session, website_setting_rule, cur_body_page, next_page_data, PlaywrightBodyPageLoader.fetch_html)
        return next_page_data


class PlaywrightIndexPageLoader(IndexPageLoader):
    @staticmethod
    async def fetch_html(session: playwright_websession.WebSession, website_setting_rule: WebsiteSettingRule, index_page_data: IndexPageData) -> None:
        return await _fetch_html(session, website_setting_rule, index_page_data)
    ...
    @staticmethod
    async def goto_next_page(session: playwright_websession.WebSession, website_setting_rule: WebsiteSettingRule, cur_index_page: IndexPageData) -> IndexPageData:
        next_page_data = IndexPageData(url=cur_index_page.next_page_url, page_id=cur_index_page.page_id + 1)
        await _goto_next_page(session, website_setting_rule, cur_index_page, next_page_data, PlaywrightIndexPageLoader.fetch_html)
        return next_page_data
